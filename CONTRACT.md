...
body: { idToken: string }
res 200: { status: "success", message: "Login Google berhasil broskie!", data: { token: string, user: { id: int, nama: string, email: string, googleId: string | null, username: string | null, noHp: string | null, bio: string | null, avatarUrl: string | null, latitude: float | null, longitude: float | null, level: int, exp: int, streak: int, lastAktivitas: string | null } } }
...
- **res 200:** `{ "status": "success", "message": "Login berhasil.", "data": { "token": string, "user": { "id": int, "nama": string, "email": string, "username": string | null, "noHp": string | null, "bio": string | null, "avatarUrl": string | null, "latitude": float | null, "longitude": float | null, "level": int, "exp": int, "streak": int, "lastAktivitas": string | null } } }`
...
- **res 200:** `{ "status": "success", "data": { "id": int, "nama": string, "email": string, "username": string | null, "noHp": string | null, "bio": string | null, "avatarUrl": string | null, "latitude": float | null, "longitude": float | null, "streak": int, "level": int, "exp": int } }`
- **res 401/404:** `{ "status": "fail", "message": "..." }`
...
- **res 200:** `{ "status": "success", "message": "Profil berhasil diupdate!", "data": { "id": int, "nama": string, "email": string, "username": string | null, "noHp": string | null, "bio": string | null, "avatarUrl": string | null, "latitude": float | null, "longitude": float | null, "streak": int, "level": int, "exp": int } }`
- **res 400/409:** `{ "status": "fail", "message": "..." }`
...
- **res 201:** `{ "status": "success", "message": "Aktivitas dicatat! Tanaman dapet +X poin. Kamu dapat +Z EXP. Streak kamu: Y hari 🔥!", "data": { "log": { "id": int, "userId": int, "tanamanId": int, "tipeValidasi": "button_only" | "photo", "fotoUrl": string | null, "createdAt": string(ISO) }, "skorSaatIni": int, "streak": int, "expDidapat": int, "expSekarang": int, "level": int, "levelUp": boolean } }`
- **catatan EXP:** `expDidapat` = tambahan exp dari aktivitas ini (+2 untuk button_only, +5 untuk photo). `expSekarang` = total exp user saat ini. `level` = level user saat ini. `levelUp` = `true` jika user naik level setelah aktivitas ini.
...
- **res 201:** `{ "status": "success", "message": "Panen berhasil dicatat broskie! 🌾", "data": { "id": int, "userId": int, "tanamanId": int | null, "namaTanaman": string, "jenisTanaman": string, "tanggalPanen": string(ISO), "expDidapat": int, "expSekarang": int, "level": int, "levelUp": boolean } }`
- **catatan EXP:** Saat panen berhasil, user mendapat tambahan EXP panen. `expDidapat` = jumlah EXP yang didapat (+30), `expSekarang` = total EXP saat ini, `level` = level terbaru, `levelUp` = true jika naik level.
...
- **res 200:** `{ "status": "success", "data": { "id": int, "userId": int, "user_nama": string, "tipePost": string, "deskripsi": string, "fotoUrl": string | null, "createdAt": string(ISO), "latitude": float, "longitude": float, "jumlahKomentar": int, "jumlahLike": int, "isLiked": boolean, "isOwner": boolean } }`
- **res 404:** `{ "status": "fail", "message": "Postingan tidak ditemukan." }`

### GET /api/community/:id/komentar
- **query:** `?page=1&limit=20`
- **res 200:** `{ "status": "success", "meta": { "halamanSekarang": int, "dataPerHalaman": int }, "data": [ { "id": int, "userId": int, "userNama": string, "userAvatar": string | null, "teks": string, "createdAt": string(ISO) } ] }`

### POST /api/community/:id/komentar
- **req (JSON):** `{ "teks": string (max 300) }`
- **res 201:** `{ "status": "success", "message": "Komentar berhasil ditambahkan!", "data": { "id": int, "userId": int, "userNama": string, "userAvatar": string | null, "teks": string, "createdAt": string(ISO) } }`
- **res 400:** `{ "status": "fail", "message": "Komentar tidak boleh kosong broskie." }` atau max 300 karakter.

### DELETE /api/community/komentar/:id
- **res 200:** `{ "status": "success", "message": "Komentar berhasil dihapus." }`
- **res 403:** `{ "status": "fail", "message": "Ini bukan komentarmu broskie!" }`
- **res 404:** `{ "status": "fail", "message": "Komentar tidak ditemukan." }`

### POST /api/community/:id/like
- **res 200:** `{ "status": "success", "data": { "liked": boolean, "jumlahLike": int } }`
...
### POST /api/tanibot
- **req (JSON):** `{ "pertanyaan": string }`
- **res 200:** `{ "status": "success", "message": "Berhasil mendapatkan jawaban TaniBot", "data": { "jawaban": string } }`
- **catatan:** TaniBot dilengkapi dengan memori *multi-turn* (mengingat percakapan sebelumnya) dan disuntikkan konteks personal secara dinamis (mengetahui data tanaman user, status penyiraman, sisa hari panen, cuaca terkini, level, dan streak) sehingga jawabannya sangat personal dan akurat. TaniBot juga dipagar khusus (Topic Guard) untuk HANYA merespons topik seputar pertanian dan akan menolak topik di luar itu dengan ramah.
...
---
## 🌟 SISTEM EXP & LEVEL
- **Validasi Siram (Button Only):** `+2 EXP`
- **Validasi Siram (Photo):** `+5 EXP`
- **Panen Tanaman:** `+30 EXP`
- **Rumus Level:** `level = Math.floor(exp / 100) + 1` (Setiap kelipatan 100 EXP = naik 1 Level).
- **Aturan Level:** Level tidak pernah turun (anti demosi) meskipun exp reset atau perhitungan lainnya.
...
6. **META BEDA BENTUK antar endpoint — jangan bikin helper generik**. Pada `/api/community` format `meta`-nya adalah `{ halamanSekarang, dataPerHalaman }`, sedangkan di `/api/tanibot/history` formatnya `{ page, limit, total }`.# API CONTRACT - TANISYNC
> SUMBER TUNGGAL KEBENARAN UNTUK AI FRONTEND. BACA INI, JANGAN MENGARANG.
